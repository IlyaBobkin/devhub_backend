...
export default {
  ...
  createVacancyInvitation: async (res, { id, companyOwnerId, applicantId, vacancyId, message }) => {
    ...
    try {
      ...
      // Создаём уведомление для соискателя
      const notificationMessage = `Вас пригласили на вакансию!`;
      await dbController.createNotification(
        applicantId,
        'invitation',
        notificationMessage,
        vacancyId,
  ...
  updateVacancyResponseStatus: async (res, { responseId, status }) => {
    ...
    try {
      const responseQuery = `
        SELECT vr.*, v.id AS vacancy_id, v.title AS vacancy_title, u.id AS applicant_id
        FROM vacancy_responses vr
        JOIN vacancies v ON vr.vacancy_id = v.id
        JOIN users u ON vr.user_id = u.id
        WHERE vr.id = $1;
      ...
      const message = status === 'accepted'
        ? `Ваш отклик на вакансию принят!`
        : status === 'declined'
        ? `Ваш отклик на вакансию отклонен.`
        : `Статус вашего отклика изменен на: ${status}.`;
      await dbController.createNotification(
  ...
  // Новый метод для создания уведомления
  createNotification: async (userId, type, message, vacancyId, responseId = null, invitationId = null) => {
    const client = await pool.connect();
    try {
      ...
      const query = `
        INSERT INTO notifications (id, user_id, type, message, vacancy_id, response_id, invitation_id, fcm_token)
        VALUES ($1, $2, $3, $4, $5, $6, $7, $8)
        RETURNING *;
      `;
      const values = [randomUUID(), userId, type, message, vacancyId, responseId, invitationId, fcmToken];
      const result = await client.query(query, values);
      const notification = result.rows[0];

      // Отправляем push-уведомление, если есть FCM-токен
      if (fcmToken) {
        await sendPushNotification(fcmToken, 'Новое уведомление', message);
      }
  ...
  // Новый метод для получения уведомлений пользователя
  getUserNotifications: async (res, userId) => {
    const client = await pool.connect();
    try {
      const query = `
        SELECT n.*, v.title as vacancy_title
        FROM notifications n
        LEFT JOIN vacancies v ON n.vacancy_id = v.id
        WHERE n.user_id = $1
  ...
  // Новый метод для сохранения FCM-токена
  saveFcmToken: async (res, userId, fcmToken) => {
    const client = await pool.connect();
    try {
      const query = `
        UPDATE users
        ...
        RETURNING *;
      `;
      const result = await client.query(query, [fcmToken, userId]);
      if (!result.rows.length) {
        return res.status(404).json({ error: 'User not found' });
      }
      res.json(result.rows[0]);
